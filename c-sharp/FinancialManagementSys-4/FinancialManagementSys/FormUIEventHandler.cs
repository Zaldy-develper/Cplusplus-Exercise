...
namespace WinFormsApp1SQL
{
    ...
    abstract class GridDataViewBaseEventHandler : IFormUIEventHandler, IDataGridEntry
    {
                                                        ...
                                                        DateTimePicker dateTimePickerEnd)
        {
            ...
            int transactionID = Convert.ToInt32(currentDGV.CurrentRow.Cells["TransactionID"].Value);
            string activityType = currentDGV.CurrentRow.Cells["ActivityType"].Value.ToString();
            decimal amount = Convert.ToDecimal(currentDGV.CurrentRow.Cells["Amount"].Value);
            DateTime transactionDate = Convert.ToDateTime(currentDGV.CurrentRow.Cells["TransactionDate"].Value);
            string transactionType = currentDGV.CurrentRow.Cells["TransactionType"].Value.ToString();

            // Retrieve new fields.
            string budgetGroup = currentDGV.CurrentRow.Cells["BudgetGroup"].Value?.ToString();
            string paymentMethod = currentDGV.CurrentRow.Cells["PaymentMethod"].Value?.ToString();
            string payee = currentDGV.CurrentRow.Cells["Payee"].Value?.ToString();
            string recurrence = currentDGV.CurrentRow.Cells["RecurrenceIndicator"].Value?.ToString();
            string currency = currentDGV.CurrentRow.Cells["Currency"].Value?.ToString();
            string description = currentDGV.CurrentRow.Cells["Description"].Value?.ToString();

            // Create an instance of EditTransactionForm, passing all values.
            using EditTransactionForm editForm = new EditTransactionForm(
                transactionID, activityType, amount, transactionDate, transactionType,
        ...
        public virtual bool IsFillupComplete(DataGridView grid)
        {
            ...
            foreach (DataGridViewRow row in grid.Rows)
            {
                ...
                if (!string.IsNullOrWhiteSpace(activity) ||
                    ...
                    !string.IsNullOrWhiteSpace(budgetGroup) ||
        ...
        public virtual bool IsFillupValid(DataGridView grid, DataGridViewRow row)
        {
            ...
            // Retrieve values for the additional columns.
            _budgetGroup = row.Cells["BudgetGroup"].Value?.ToString();
            _paymentMethod = row.Cells["PaymentMethod"].Value?.ToString();
            _payee = row.Cells["Payee"].Value?.ToString();
            _recurrence = row.Cells["Recurrence"].Value?.ToString();
            _currency = row.Cells["Currency"].Value?.ToString();
            ...
            if (string.IsNullOrWhiteSpace(_activity) ||
                ...
                string.IsNullOrWhiteSpace(_budgetGroup) ||
                string.IsNullOrWhiteSpace(_paymentMethod) ||
                string.IsNullOrWhiteSpace(_payee) ||
                string.IsNullOrWhiteSpace(_recurrence) ||
