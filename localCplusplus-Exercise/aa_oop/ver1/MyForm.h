#pragma once
#include "ValidationFlow.h" // Contains InputHandler, ValidationBlock, runFlow(), etc.
#include "ValidationFlowConfig.h"  // Contains setupValidationBlocks()
...
#pragma endregion
	...
	private: System::Void btnStartreceive_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		CustomInput input(ip, port, path); // Assign my Custom Input
		std::string errorMessage;

		// Build the validation flow using the builder configuration.
		std::vector<std::shared_ptr<ValidationBlock>> blocks = setupValidationBlocks();

		// For demonstration: show that the starting block is initialized.
		for (size_t i = 0; i < blocks.size(); ++i) {
			/*std::cout << "Block[" << i << "] initialized." << std::endl;*/
			ReceiveStatus("Block[" + i + "] initialized. Total Block size is " + blocks.size(), textboxStatus);
		}

		// Execute the validation flow starting from the first block.
		if (!blocks.empty()) {
			runFlow(blocks[0], input, errorMessage);
		}
		...
		if (errorMessage.empty()) {
			...
			ReceiveStatus("Input Validation succeeded.", textboxStatus);
		}
		else {
			if (!errorMessage.empty()) {
				// Report the error.
