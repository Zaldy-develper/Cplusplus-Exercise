...
#pragma endregion
	...
	private: System::Void btnStartreceive_Click(System::Object^ sender, System::EventArgs^ e) {
		 ...
		 *			@																*
		 *==========================================================================*/
		const int MAX_CLIENTS = 3;  // Example: accept three clients

		// Declare an array of SocketHandler^ (managed pointers)
		array<SocketHandler^>^ multisockethandler = gcnew array<SocketHandler^>(MAX_CLIENTS);

		for (int i = 0; i < MAX_CLIENTS; i++)
		{
			// Accept a client connection.
			multisockethandler[i] = gcnew SocketHandler(port);
			multisockethandler[i]->StartServer(textboxStatus, textboxFilepath);

			// Create a handler for this client.
			ClientHandler^ handler = gcnew ClientHandler(multisockethandler[i]->GetNewSocket(), textboxFilepath, textboxStatus);

			// Create and start a new thread that will run the handler's HandleClient method.
			Thread^ clientThread = gcnew Thread(gcnew ThreadStart(handler, &ClientHandler::HandleClient));
			clientThread->Start();

			//Start File Transfer
			multisockethandler[i]->ReceiveFile(multisockethandler[i]->GetNewSocket(), textboxFilepath, textboxStatus) ? true : false;

			ClosingCleanUp^ cleanup = gcnew ClosingCleanUp(multisockethandler[i]->GetNewSocket(), multisockethandler[i]->GetServerFd());
			cleanup->Close();
		}
