...
#pragma endregion
	...
	private: System::Void btnStartreceive_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		SocketHandler^ sockethandler = gcnew SocketHandler(
			address,
			addrlen,
			server_fd,
			port,
		...
		if (sockethandler->IsSocketDescriptorFailed()) {
			ReceiveStatus(sockethandler->LastError, textboxStatus);
			return;
		}
		if (sockethandler->IsBindingFailed()) {
			ReceiveStatus(sockethandler->LastError, textboxStatus);
			return;
		}
		else {
			ReceiveStatus(sockethandler->LastSuccess, textboxStatus);
		}
		if (sockethandler->IsNotListening()) {
			ReceiveStatus(sockethandler->LastError, textboxStatus);
			return;
		}
		else {
		...
		if (sockethandler->IsClientNotAccepting()) {
			ReceiveStatus(sockethandler->LastError, textboxStatus); // Print error message on Receive Status Textbox
			// Print Error message on Notepad
			sockethandler->StatusUpdate(sockethandler->LastStatus, textboxFilepath) ?
				true : ReceiveStatus(sockethandler->LastError, textboxStatus); // Print error Writing message on Notepad
			return;
		}
		else {
			ReceiveStatus(sockethandler->LastSuccess + sockethandler->GetNewSocket().ToString(), textboxStatus);
			// Print Client Accepted message on Notepad
			sockethandler->StatusUpdate(sockethandler->LastStatus, textboxFilepath) ?
				true : ReceiveStatus(sockethandler->LastError, textboxStatus); // Print error Writing message on Notepad
			ReceiveStatus("Trying to send from server to Client...", textboxStatus);
			(sockethandler->TestMessageSendToClient(sockethandler->GetNewSocket()) ? 
				ReceiveStatus(sockethandler->LastSuccess, textboxStatus) : ReceiveStatus(sockethandler->LastError, textboxStatus));
		}
		...
		ReceiveFile(sockethandler->GetNewSocket(), textboxFilepath->Text, textboxStatus) ? true : false;
		...
		ClosingCleanUp^ closingCleanUp = gcnew ClosingCleanUp(sockethandler->GetNewSocket(), server_fd);
		closingCleanUp->Close();
