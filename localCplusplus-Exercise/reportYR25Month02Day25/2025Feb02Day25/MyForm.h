...
#pragma endregion
	...
	private: System::Void btnStartreceive_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		String^ message = String::Join(Environment::NewLine, params); // for deletion
		...
		if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
			ReceiveStatus("ERROR: WSAStartup failed");
			return;
		}
		...
		if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
			...
			ReceiveStatus("ERROR: socket failed");
			exit(EXIT_FAILURE);
		}
		else ReceiveStatus("ソケットファイルディスクリプタの作成しました。");
